...
void solve() {
    int n;
    ll k; // ll is already defined as long long at line 42
    cin >> n >> k;
    
    ll arr[101];
    rep(i,0,n) cin >> arr[i]; // rep = for loop
    
    // if any number divisible by k "YES"
    rep(i,0,n){
        if(arr[i]%k==0){
            cout << "YES" << endl;
            return ;
    ...
    // multiplying all arr[i] with modulo k
    ll product = 1;
    rep(i,0,n){
        product = (product*arr[i])%k;
    }
    
    if(product) cout << "NO" << endl;
    else cout << "YES" << endl;
